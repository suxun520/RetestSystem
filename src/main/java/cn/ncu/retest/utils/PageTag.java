...
public class PageTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		sb.append(
				".pagination a, .pagination a:link, .pagination a:visited {padding:2px 5px;margin:2px;border:1px solid #b5b5b5;text-decoration:none;color:#006699;}");
		sb.append(".pagination a:hover, .pagination a:active {background:#fff; text-decoration:none;}");
		sb.append(
				".pagination span.current {padding: 2px 5px;margin: 2px;border: 1px solid #b5b5b5;font-weight: bold;background-color: #fdfdfd;box-shadow:1px 4px 6px rgba(0,0,0,0.2) inset;}");
		...
		if (this.totalRecord == 0) {
			...
			sb.append("<input type=\"hidden\" name=\"").append("pageNo").append("\" value=\"").append(this.pageNo)
					.append("\" />\r\n");
			...
			if (pageNo == 1) {
				...
				sb.append("<a href=\"javascript:turnOverPage(").append(this.pageNo - 1).append(")\" >上一页</a>\r\n");
			}
			...
			if (this.pageNo == pageCount) {
				...
				sb.append("<a href=\"javascript:turnOverPage(").append(this.pageNo + 1).append(")\" >下一页</a>\r\n");
			}
